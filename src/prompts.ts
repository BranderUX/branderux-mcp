...
4. update_brand_settings with a coherent palette and type (and the site's logo URL as iconUrl when it exists, logoHasWordmark: true when that logo spells the name); update_project_settings with customPages — 3-5 nav entries ({id, name, query}) matching the screens, so the embed has navigation. Projects serve FLEXIBLE mode by default — leave uiGenerationMode unset; write it only when the owner asks for something that needs the other mode.
...
1. read_doc "hosted-agent-contract" FIRST (also "custom-elements-contract" and "screens-wire-format" before authoring). Then read_doc "hosted-design-bar" and list_templates / get_template for the reference build whose MOMENT is closest to this business — a worked example to adapt (their moment, their data, their voice), never a kit to copy.
2. Ask me the FIVE MANDATORY QUESTIONS from the contract (login requirement, access follow-up, handoff email — never my account email, escalation timing, write-tool consent) and WAIT for my answers. Plus the contract's conditional marketing-consent question whenever an entity of mine collects a phone number or an email address: whether I plan to send offers or news to those people — a yes adds the marketingConsent field and has the agent ask each visitor for it, a no leaves the list service-only, and tell me which one I ended up with.
3. whoami → create_project → brand.
4. define_entity for the real data shapes — writable entities (bookings/orders/enquiries) get the right writePolicy per the coherence rule and are end-user-scoped, never public-read. Every property of an intake entity carries a title in the site's language — the field name the visitor reads on the Confirm card; description is for the model. On an EXISTING project run list_entities first and re-define any intake entity that is public-read as end-user-scoped (same jsonSchema) before anything else. Then seed_records (marked _demo) or wire live sources.
5. upsert_agent_config: persona in the business's voice encoding my escalation-timing answer; policies from my answers (handoff email = the address I typed, it activates escalate_to_owner — real owner email) plus policies.language and policies.timezone (always) and policies.entityLabels for a non-English site; upsert_skill for real domain knowledge. Ask me here for the registered name of my business (policies.legalName) and for one email or phone that privacy requests should reach (policies.noticeContact), and store exactly what I answer: my site's privacy notice shows both, so never take either from my website or guess one, and if I skip a question store nothing for it and tell me the notice is missing that detail. Never ask me about answer quality or which AI model to use — the balanced default is right; change the stop (upsert_agent_config level 1-5) only if I explicitly ask for faster, cheaper or smarter answers, and never name a model, a vendor or a price. If I bring up using my own AI provider key: never ask for it, read it, repeat it or put it in this conversation — send me to the "Your API key" card under Advanced in the Agent tab's Answer quality panel (or, when you are the in-app Builder, open its secure field with request_credential named model-<provider>), and if I paste a key here tell me to remove it and use the card.
...
7. put_screen the screens, update_project_settings customPages, then set_home_screen (required). Give the questions I get asked over and over (my menu, my price list, my opening hours, what is new this week, a size guide) a fixed screen each with set_fixed_screens, up to twelve of them, so each one comes back instantly and identically on any turn, with live rows wherever my data is live. A chip, a page of mine or a link whose words are one of those screens' words is answered by that screen before any AI runs: my visitor's own words go into the conversation and my designed screen comes back under them, so the wording on the chip and the wording of the screen have to be identical, character for character, or the screen never shows. If what you are building for me is the chat bubble on my own site (the floating widget, whether my site is React or Wix, WordPress, Shopify, Squarespace or plain HTML) and I have not told you what my home should be, then my home is the welcome text exactly as it is plus ONE queries list of the questions my visitors actually ask: plain chips, and a featured block on live rows only if I have a catalogue. A chip that collects details before it fires (a table, an appointment, a quote) belongs on that list only when that action is the thing my business exists to do; a shop with a catalogue gets no such chip, because buying happens on my site, and leaving details for a callback stays inside the conversation as the fallback for when the assistant cannot help. Name what answers each question and drop any question nothing can answer. That default is off for my whole-site build and for a page or panel inside my site, and anything I have told you about my home wins over it. If you cannot tell where this will run, ask me once: as a chat bubble on my site, as a page inside my site, or as the whole site? Every time you store my home or one of those screens, read the verification that comes back with it: it runs each block of the screen against my real data, and a screen that is not ok will be answered by the live agent instead of the page you designed, so fix what it names (point it at the right data or loosen the filter) or mark a side block optional, never the main list of the screen, and check again. The same report names, per screen, the questions on it that nothing canned answers: every one of those is answered live, so leave it live only where one of my skills covers it on purpose or where the chip itself collects the details and places the booking or the order, and give each of the others a fixed screen. My booking, order or quote form itself is never one of those fixed screens, even when a chip opens it: a fixed screen comes back blank, while the live agent opens my form already filled in with what the visitor told it and, when they are signed in, who they are. A probe of an API is never proof; only that verification is. Run verify_canned_screens before you publish and tell me in one sentence if my store is blocking you (it answers 403): I need to allow requests whose User-Agent contains BranderUX-Connector/1.0 to my store's API path, which on Cloudflare is a skip rule.
8. list_entities to VERIFY writePolicy round-tripped, then publish_site IMMEDIATELY — derive the slug from the business name, announce the live URL, remind me that writes and emails run there and in the widget on my site, and sign-in on the site itself.
